...
class Wordframe(tk.Frame):
    def __init__(self, root, native, translation, progress, status, sentence):
        super().__init__(root, width=650, height=60)  
        ...
        # Use pack with appropriate options to stack labels vertically
        ...
        self.sentence = ctk.CTkLabel(self, text=f"{sentence}")
        self.sentence.place(x=250, y=17.5)

        self.delete_button = ctk.CTkButton(self, text="Delete", command=self.delete_word, hover_color="red")
        self.delete_button.place(x=750, y=17.5)

        self.B_train_card = ctk.CTkButton(self, text="Train", command=self.train, hover_color="green")
        self.B_train_card.place(x=600, y=17.5)
